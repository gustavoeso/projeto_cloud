...
O projeto de Cloud consiste em construir uma arquitetura de cloud utilizando a plataforma AWS da Amazon. O projeto proposto é criar uma integração entre um Aplication Load Balancer com um número de VPCs (virtual private cloud) que deve ser controlado por um AutoScaling para gerenciar o número de VPCs a serem utilizadas pelo sistema. Com isso as VPCs devem todas conectar em uma única base de dados DynamoDB, e ter uma aplicação que consiga controlar a base de dados. Para isso as instâncias são carregadas com uma AMI (imagem) obtida pela AWS no recurso de EC2, onde tem um catalogo com todas as imagens Disponíveis, no caso do projeto, foi utilizado Amazon Linux onde após algumas pesquisas, parecia ser o mais recomendado para a escala do projeto e o tipo de aplicação que seria utilizada. O projeto também utiliza 2 subnets, isso por que existia uma exigência da amazon para subir a stack. Já a aplicação, para conseguir colocar nas VPCs, foi utilizado um recurso chamado Bucket da AWS. Ele é essencialmente um container, onde ele esta jogando a aplicação para a cloud, e assim que as máquinas são criadas, elas podem baixar essa aplicação pelo terminal (user data do LaunchConfiguration) e rodar a aplicação. O DynamoDB foi configurado de uma forma onde a tabela se chama "Users", onde seu conteúdo é UserID do tipo String, e ele é a chave primária.

Na imagem 1 pode-se observar como o projeto está estruturado com o Aplication Load Balancer, as VPCs e a base de dados. No diagrama são destacados 3 VPCs, isso por que o programa feito tem como máquinas desejadas por padrão 3, porém, com o AutoScaling, ele pode variar o número de VPCs de 1 a 5 dependendo do uso da aplicação. O AutoScailing funciona a partir de um alarme, onde se o uso estiver abaixo de 70%, ele diminue o número de máquinas sendo utilizadas, mas se passar desse "threshold", ele sobe o número de máquinas, no caso do projeto de 1 a 5 como mencionado anteriormente.
